...
function StatusBadge({ status }: { status: string }) {
  ...
  return (
    <span style={{
      background: s.bg, color: s.color, borderRadius: 20, padding: "3px 11px",
      ...
      {Array.from({ length: max }, (_, i) => i + 1).map(n => (
        <span key={n}
          ...
          style={{
            ...
            color: n <= (hov || value) ? C.blue : C.muted,
...
function InputField({ label, children }: { label: string; children: React.ReactNode }) {
  return (
    <div style={{ marginBottom: 14 }}>
      <label style={{
        display: "block", fontFamily: FONT, fontWeight: 700, fontSize: 12,
        color: C.sub, marginBottom: 7, textTransform: "uppercase", letterSpacing: ".6px",
      }}>{label}</label>
      {children}
    </div>
  );
      ...
      <Card style={{ marginBottom: 14 }}>
        <InputField label="Item Name *">
          <input value={f.itemName} onChange={e => set("itemName", e.target.value)}
            placeholder="e.g. Sony Headphones" style={inputStyle} />
        </InputField>
      ...
      {mine.length === 0
        ...
        : mine.map(w => (
          <Card key={w.id} style={{ marginBottom: 14 }}>
            ...
            {w.status !== "Redeemed"
              ? <Btn variant="soft" onClick={() => onRequestAgain(w.id)} style={{ fontSize: 13, padding: "9px 18px" }}>
                  🔁 Request Again ({w.requestCount}x)
                </Btn>
        ...
        : pending.map(w => (
          <Card key={w.id} style={{ marginBottom: 18 }}>
            ...
            <div style={{ display: "flex", gap: 10, alignItems: "center", flexWrap: "wrap", marginBottom: 10 }}>
              <HeartBar value={w.desireLevel} />
              <span style={{ fontSize: 13, color: C.sub, fontWeight: 600 }}>{money(w.price)}</span>
              <span style={{ fontSize: 12, background: C.blueLight, color: C.blueDark, borderRadius: 12, padding: "2px 9px", fontWeight: 700 }}>
                {w.requestCount}x requested
              </span>
            </div>
            {w.url && (
              <a href={w.url} target="_blank" rel="noreferrer"
                style={{ fontSize: 13, color: C.blue, fontWeight: 700, display: "block", marginBottom: 10 }}>
                🔗 View Product
              </a>
            )}
            {w.persuasionText && (
            ...
            <div style={{ marginBottom: 14 }}>
              <div style={{ fontWeight: 700, fontSize: 12, color: C.sub, marginBottom: 8, textTransform: "uppercase", letterSpacing: ".5px" }}>
                Assign Star Cost (1–10)
              </div>
            ...
            <div style={{ display: "flex", gap: 8 }}>
              <Btn variant="muted" onClick={() => handle(w.id, "Considering")}
                disabled={saving === w.id + "Considering"} style={{ fontSize: 13, padding: "9px 16px" }}>
                🧠 Considering
              </Btn>
              <Btn onClick={() => handle(w.id, "Approved")}
                disabled={saving === w.id + "Approved"} style={{ fontSize: 13, padding: "9px 16px" }}>
                🚀 Approve Wish
              </Btn>
            </div>
          </Card>
...
function RedeemPage({ currentUser, wishes, onBack, onRedeem }: { currentUser: User; wishes: Wish[]; onBack: () => void; onRedeem: (id: string) => Promise<void> }) {
  ...
  return (
    <div>
      ...
      <div style={{
        background: `linear-gradient(135deg, ${C.blue} 0%, #C8D8F0 100%)`,
        borderRadius: 18, padding: "18px 22px", marginBottom: 20, color: C.white,
      ...
      <div style={{ display: "flex", gap: 8, marginBottom: 20 }}>
        {["All", "Approved", "Considering"].map(t => (
          <button key={t} onClick={() => setTab(t)} style={{
            padding: "9px 18px", borderRadius: 50, fontFamily: FONT, fontWeight: 700, fontSize: 13,
            cursor: "pointer", border: `2px solid ${tab === t ? C.blue : C.border}`,
  ...
  return (
    <div>
      ...
      <div style={{ display: "grid", gridTemplateColumns: "1fr 1fr", gap: 12, marginBottom: 20 }}>
        {[currentUser, partner].filter(Boolean).map((u, i) => u && (
          <Card key={u.id} style={{ textAlign: "center", padding: "20px 12px", border: i === 0 ? `2px solid ${C.blue}` : `1px solid ${C.border}` }}>
            <div style={{
              width: 40, height: 40, borderRadius: "50%",
              background: i === 0 ? C.blue : C.pink,
              color: i === 0 ? C.white : C.pinkDark,
      ...
      <div style={{
        ...
        <div style={{ fontSize: 13, color: C.white, fontWeight: 700, opacity: .85, marginBottom: 12 }}>
          Show {partner?.name} some love!
        </div>
        ...
        : sorted.map(a => (
          <Card key={a.id} style={{ marginBottom: 10, display: "flex", alignItems: "flex-start", gap: 12, padding: "14px 16px" }}>
            <div style={{
              ...
              background: avatarBg[a.actorId] || C.blueLight,
              display: "flex", alignItems: "center", justifyContent: "center",
...
function BottomNav({ page, onNavigate }: { page: string; onNavigate: (page: string) => void }) {
  return (
    <nav style={{
      ...
      padding: "8px 4px 12px", boxShadow: "0 -4px 24px rgba(180,180,160,0.12)", zIndex: 100,
    }}>
      {NAV_ITEMS.map(({ page: p, icon, label }) => {
        ...
        return (
          <button key={p} onClick={() => onNavigate(p)} style={{
            ...
            <span style={{ fontFamily: FONT, fontSize: 10, fontWeight: active ? 800 : 600, color: active ? C.blue : C.muted }}>
              {label}
            </span>
